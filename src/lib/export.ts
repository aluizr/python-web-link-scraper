...
export function exportAsCSV(links: LinkItem[]): Blob {
  const headers = ['Title', 'URL', 'Category', 'Tags', 'Favorite', 'Status', 'Priority', 'Due Date', 'Description', 'Notes', 'Created At'];
  ...
  const rows = links.map(link => [
    ...
    escapeCSV(link.priority),
    escapeCSV(link.dueDate || ''),
                ...
                ${links
                  .map(
                    ...
                    <tr>
                        ...
                        <td>${escapeHtml(link.priority)}</td>
                        <td>${link.dueDate ? new Date(link.dueDate).toLocaleDateString('pt-BR') : '-'}</td>
