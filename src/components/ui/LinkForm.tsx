...
export function LinkForm({ open, onOpenChange, categories, links, editingLink, onSubmit, onEditDuplicate }: LinkFormProps) {
  ...
  // ✅ Fix 1: ogImage aceita string | null para representar ausência de imagem
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (!editingLink) {
        // ✅ Fix 2: usar null explicitamente é válido agora que ogImage é string | null
        setOgImage(null);
        setTitle("");
        setDescription("");
  ...
  // ✅ Fix 3: substituído `isEditing` (inexistente) por `!!editingLink`
  const handleFetchMetadata = async (targetUrl: string) => {
    if (!targetUrl || !!editingLink) return;
